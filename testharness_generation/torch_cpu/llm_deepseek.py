...
Uses DeepSeek-V4-Pro through an OpenAI-compatible Chat Completions endpoint.

This file keeps the original FlashFuzz prompt construction and harness
generation pipeline unchanged. Only the LLM backend is replaced.
...
def call_deepseek(
    ...
    *,
    ...
    """Call DeepSeek-V4-Pro through an OpenAI-compatible API.

    Retries on timeouts, connection errors, HTTP 5xx/429, and specific
    gateway timeouts. Falls back across multiple API bases.
    """
        ...
        for attempt in range(1, max(1, retries) + 1):
            try:
                ...
                try:
                    ...
                    snippet = ""
                print(f"[ERROR] DeepSeek call failed for {api_name}: HTTP {status} at {base}")
                if snippet:
                ...
                if retriable and attempt < max(1, retries):
                    ...
                    delay *= 1.6
                    continue
                print(f"[ERROR] DeepSeek call failed for {api_name}: {e}")
                break
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Generate C++ test harnesses via DeepSeek-V4-Pro")
    ...
    p.add_argument(
        ...
        help="Bearer token for DeepSeek API"
    )
